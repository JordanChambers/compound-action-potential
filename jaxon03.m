...
%%%       - jfac6b - reduce the number of lamellae by this many
    ...
    jfac1 = jfac1b + 0.25*randn(1,jreps);%node diameter and internode diameter %% 0.61 + 0.37 (using 0.25)
    jfac2 = jfac2b + 0.1*randn(1,jreps);%node length %% using 1+0.1
    jfac3 = jfac3b + 0*randn(1,jreps);
    jfac4 = jfac4b + 0.1*randn(1,jreps);%internode length %% using 1+0.1
    jfac5 = jfac5b + 0.05*randn(1,jreps);%g-ratio %% 0.7025 + 0.0356; 0.7266 + 0.0240
    jfac7 = 1 + 0.1*randn(1,jreps);%0.1
    jfacpasv = jfacpas + 0.25*randn(1,jreps);%0.25
    jfacpasv(jfacpasv<0.3) = 0.3;
    

    jabc = sort(jfac1);
    jfac3(:) = jabc(end-jfac3b);
    jcntfac = 0;
    for i = 1:jreps
        if jfac1(i) > jfac3(i)
            jcntfac = jcntfac + 1;
...
%mean results: velocity 2.14954, g-ratio 0.792079, number of lamellae 4, peak time 336.7 msec, half-width 214.9 msec, 0 axons without AP
%dl2ad2 = jaxon03(jval, 1, 399, 1, 1, 2);
%mean results: velocity 2.17387, g-ratio 0.792097, number of lamellae 4.16, peak time 340.4 msec, half-width 351.4 msec, 1 axons without AP
